...
function makeRepayment(uint256 repaymentId) external nonReentrant whenNotPaused onlyVerifiedClient atStatus(LoanStatus.InRepayment) {
    ...
    // For the final payment, adjust amount to cover any remaining balance exactly
    if (repaymentId == loan.totalRepayments - 1) {
        // Use the remaining balance as the final payment amount rather than the scheduled amount
        // This handles any rounding errors from the initial repayment calculation
        totalAmount = uint256(loan.remainingBalance) + uint256(penalty);
        repayment.amount = loan.remainingBalance; // Update the recorded amount to match actual payment
    } else {
        totalAmount = uint256(repayment.amount) + uint256(penalty);
    }
    
    // Ensure sufficient allowance and balance
    uint256 clientAllowance = stablecoin.allowance(client, address(this));
    require(clientAllowance >= totalAmount, "Allowance too low");
    ...
    // Update loan state
    if (repaymentId == loan.totalRepayments - 1) {
        // Ensure balance is exactly zero for final payment
    ...
    function getRepaymentPenalty(uint256 repaymentId) external view returns (uint128 penalty, uint256 weeksLate) {
        Repayment memory repayment = repayments[repaymentId];
        return _calculatePenalty(repayment.dueDate, repayment.amount);
    }
